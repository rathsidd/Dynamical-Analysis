...
def timesampled_avg_error(data,x):
    f = data[0:x,:]
    X_dmd = DMD(dt,f,r)
...
def variable_timesampled_avg_error(data,x,n):
    f = data[0:x,:]
    X_dmd = DMD(dt,f,r)
    X_dmd = X_dmd[:,0:n]
...
def variable_timesampledstepsize_avg_error(data,num_timesteps,num_samples_recon,timestep):
...
error_data = [[variable_timesampled_avg_error(f, i, j) for i in tstep]
                               for j in tstep]
...
for i in range(t_num_samples_num):
    for j in range(t_num_samples_num):
        for k in range(len(t_timestep)):
            error_data[i,j,k] = variable_timesampledstepsize_avg_error(f, 
